             ...
             <div class="grid gap-4 md:grid-cols-2">
                 <div class="relative overflow-hidden rounded-lg bg-gradient-to-br from-[#003a70] to-[#002144] p-8 text-white shadow-lg">
                     <div class="relative z-10">
                         ...
                         <button class="mt-6 rounded-md bg-[#ff6500] px-8 py-3 text-sm font-black uppercase transition hover:scale-105 hover:bg-[#e45b00]">Aproveitar agora</button>
                     </div>
        ...
        <aside class="space-y-4">
            <div class="rounded-lg bg-white p-5 shadow-sm">
                ...
                <div class="mt-4 space-y-2">
                    <button wire:click="$set('category', null)" class="flex w-full items-center justify-between rounded px-3 py-2 text-left text-[13px] transition {{ $category === null ? 'bg-[#ff6500] font-bold text-white shadow-md' : 'text-zinc-600 hover:bg-zinc-50' }}">
                    ...
                    @foreach($categories as $item)
                        <button wire:click="$set('category', {{ $item->id }})" class="flex w-full items-center justify-between rounded px-3 py-2 text-left text-[13px] transition {{ $category === $item->id ? 'bg-[#ff6500] font-bold text-white shadow-md' : 'text-zinc-600 hover:bg-zinc-50' }}">
                            <span>{{ $item->name }}</span>
            ...
            <div class="rounded-lg bg-[#002144] p-5 text-white shadow-md">
                ...
                <button class="mt-4 w-full rounded border border-white/20 py-2 text-[11px] font-black uppercase hover:bg-white/10">Saber mais</button>
            </div>
        </aside>
        ...
        <main>
            <div class="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <select wire:model.live="sort" class="rounded-md border-zinc-200 bg-white px-3 py-1.5 text-xs font-bold text-[#002144] shadow-sm focus:border-[#ff6500] focus:ring-0">
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                @forelse($products as $product)
                    <article class="group relative flex flex-col rounded-lg bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                        <div class="absolute left-4 top-4 z-10 flex flex-col gap-1">
                             <span class="rounded bg-[#ff6500] px-1.5 py-0.5 text-[9px] font-black text-white">OFERTA</span>
                             @if($product->is_featured)
                                <span class="rounded bg-[#002144] px-1.5 py-0.5 text-[9px] font-black text-white">DESTAQUE</span>
                             @endif
                        </div>

                        <a href="#" class="relative flex aspect-square items-center justify-center overflow-hidden rounded-md bg-zinc-50 transition group-hover:bg-white">
                        ...
                        <div class="mt-4 flex flex-1 flex-col">
                            ...
                            <div class="mt-6 flex flex-col gap-2">
                                <button
                                    ...
                                    class="flex w-full items-center justify-center gap-2 rounded bg-[#ff6500] py-2.5 text-xs font-black uppercase text-white shadow-lg shadow-orange-500/20 transition-all hover:bg-[#e45b00] active:scale-95"
                                >
                                ...
                                <button
                                    ...
                                    class="flex w-full items-center justify-center gap-2 rounded border-2 border-[#002144] py-2 text-xs font-black uppercase text-[#002144] transition hover:bg-[#002144] hover:text-white active:scale-95"
                                >
                ...
                @empty
                    <div class="flex flex-col items-center justify-center rounded-xl bg-white py-20 shadow-sm sm:col-span-2 lg:col-span-3">
                        ...
                        <button wire:click="$set('search', '')" class="mt-4 text-xs font-black uppercase text-[#ff6500] hover:underline">Limpar filtros</button>
                    </div>
                @endforelse
            </div>
